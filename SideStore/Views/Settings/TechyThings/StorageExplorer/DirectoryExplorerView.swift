...
public struct DirectoryExplorerView: View {
    ...
    private func makeAlert(for alertType: StorageExplorerViewModel.ActiveAlert) -> Alert {
        ...
        case .confirmBulkDelete:
            ...
            return Alert(
                title: Text("删除选中的 \(count) 项？"),
                message: Text("确定要永久删除这 \(count) 项吗？"),
        ...
        case .pasteConflict(let conflict):
            return Alert(
                title: Text("文件已存在"),
                message: Text("此文件夹中已存在名为“\(conflict.existingName)”的项目，请输入新名称以复制："),
                primaryButton: .default(Text("复制为新名称")) {
...
private struct TrailingToolbarMenuView: View {
    ...
    var body: some View {
                ...
                ForEach(StorageSortOption.allCases) { option in
                    SwiftUI.Button {
                        ...
                        if sortOption == option {
                            Label("\(option.rawValue)（", systemImage: sortAscending ? "arrow.up" : "arrow.down")
                        } else {
        ...
        .confirmationDialog("Options", isPresented: $showTvMenu) {
            ...
            ForEach(StorageSortOption.allCases) { option in
                SwiftUI.Button("排序：\(option.rawValue)") {
                    if viewModel.sortOption == option {
                        viewModel.sortAscending.toggle()
                    } else {
                        viewModel.sortOption = option
